...
def generat_occupy_gt_lidar(base_info):
    ...
    trunc_margin = 2 * voxel_size  # truncation on SDF
    ...
    if 1:
        ...
        if img_left.shape[0] != int(resolution[1]) or \
            ...
            if 0:
                ...
                # cv2.imwrite(base_info["left_img_path"].replace(".png", "_new.png"), img_left)
                # cv2.imwrite(base_info["right_img_path"].replace(".png", "_new.png"), img_right)
                # exit(1)
        ...
        # depth_img_left[disp_img_left <= 1e-2] = 0
    ...
    # 由于点云的稀疏性， 投影到深度图的时候有可能取不到深度，设定一个像素的范围，取该范围内最近点的深度
    cam_num = cam_pts.shape[0]
    for i in range(cam_num):
        # 该grid的中心是在图像内的，然后这个grid是取不到深度的
        if valid_pix[i] and depth_val[i] == 0:
            parse_pix_x = pix_x[i]
            parse_pix_y = pix_y[i]

            # 设置搜索的像素范围
            search_pix = 20
            for i in range(1, search_pix):
                xmin = parse_pix_x - i
                xmax = parse_pix_x + i
                ymin = parse_pix_y - i
                ymax = parse_pix_y + i
                ...
                search_valid_num = np.sum(depth_img_left[ymin:ymax, xmin:xmax] != 0)
                if search_valid_num != 0:
                    search_valid_depth = np.sum(depth_img_left[ymin:ymax, xmin:xmax])/search_valid_num
                    depth_val[i] = search_valid_depth
                    break
