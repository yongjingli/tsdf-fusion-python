...
def generat_occupy_gt_lidar(base_info):
    ...
    # 将cam_pts扩充为8个角点，将8个角点取到深度的平均作为该grid的取到深度，当取为0时不进行计数
    # 依然选择grid的中心作为该grid的代表，然后与上面取到的深度进行对比，从而判断类型
    # 该方法不太行，一个grid的8个角点投影到图像上的位置差异还是挺大的！！！！！
    pix_z = cam_pts[:, 2]
    ...
    # 安装box中心进行扩大
    dx_dy_dz = np.ones((cam_num, 3), dtype=cam_pts.dtype) * voxel_size * 0.1
    angle = np.zeros((cam_num, 1), dtype=cam_pts.dtype)
    corner_cam_pts = np.concatenate([cam_pts, dx_dy_dz, angle], axis=1)
    corner_cam_pts = get_corners(corner_cam_pts)
    # (N, 8, 3) -> (N*8, 3)
    corners_cam_pts = corner_cam_pts.reshape(-1, 3)

    corner_pix = cam2pix(corners_cam_pts, cam_K)

    # fx, fy = cam_K[0, 0], cam_K[1, 1]
    # cx, cy = cam_K[0, 2], cam_K[1, 2]

    corner_pix_x, corner_pix_y = corner_pix[:, 0], corner_pix[:, 1]
    corner_pix_x = corner_pix_x.reshape(-1, 8)
    corner_pix_y = corner_pix_y.reshape(-1, 8)

    # cpu very slow
    depth_val = np.zeros(pix_z.shape)
    valid_pix = np.zeros(pix_z.shape)
    im_h, im_w = depth_img_left.shape

    for i in range(cam_num):
        valid_num = 0
        depth_sum = 0
        grid_center_x, grid_center_y = pix_x[i], pix_y[i]
        grid_depth = pix_z[i]
        grid_valid = np.logical_and(grid_center_x >= 0, np.logical_and(grid_center_x < im_w,
                                                                       np.logical_and(grid_center_y >= 0,
                                                                                      np.logical_and(
                                                                                          grid_center_y < im_h,
                                                                                      grid_depth > 0))))
        pix_x_min = -1
        ...
        if grid_valid:
            for j in range(8):
                cam_x, cam_y = corner_pix_x[i, j], corner_pix_y[i, j]

                cam_valid = np.logical_and(cam_x >= 0, np.logical_and(cam_x < im_w,
                                           np.logical_and(cam_y >= 0,
                                                          np.logical_and(cam_y < im_h, grid_depth > 0))))

                if cam_valid:
                    if pix_x_min != -1:
                        pix_x_min = min(pix_x_min, cam_x)
                    else:
                        pix_x_min = cam_x

                    if pix_y_min != -1:
                        pix_y_min = min(pix_y_min, cam_y)
                    else:
                        pix_y_min = cam_y

                    if pix_x_max != -1:
                        pix_x_max = max(pix_x_max, cam_x)
                    else:
                        pix_x_max = cam_x

                    if pix_y_max != -1:
                        pix_y_max = max(pix_y_max, cam_y)
                    else:
                        pix_y_max = cam_y

                    valid_pix[i] = 1
                    # img_depth = depth_img_left[cam_y, cam_x]
                    valid_num = valid_num + 1
                    # depth_sum = depth_sum + img_depth

        if valid_num != 0 and depth_sum != 0:
            if pix_x_max > pix_x_min and pix_y_max > pix_y_min:
                grid_depth_sum = np.sum(depth_img_left[pix_y_min:pix_y_max, pix_x_min:pix_x_max])
                grid_depth_num = np.sum(depth_img_left[pix_y_min:pix_y_max, pix_x_min:pix_x_max] != 0)
                depth_val_grid = grid_depth_sum/grid_depth_num
        else:
            depth_val_grid = 0
        depth_val[i] = depth_val_grid

    valid_pix = valid_pix.astype(np.bool)
